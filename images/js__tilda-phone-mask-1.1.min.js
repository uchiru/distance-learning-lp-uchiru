...
var numberArr=number.split('');var regex=/(\+|\d+|[()-]|9+)/g;var maskArr=mask.match(regex);var phoneString='';if(maskArr!=null){for(var i=0;i<maskArr.length;i++){if(maskArr[i][0]=='9'){phoneString+=numberArr.slice(0,maskArr[i].length).join('');numberArr.splice(0,maskArr[i].length)}else{phoneString+=maskArr[i]}}
phoneString=phoneString.replace(/[-()]+$/,'');phoneString=phoneString.replace(/^-+/,'');$this.val(phoneString)}}
...
function t_form_phonemask_change_min_length(mask,code,inputGroup){if(typeof code!='undefined'&&typeof mask!='undefined'){inputGroup.find('.js-phonemask-result').attr('data-tilda-rule-minlength',code.length+mask.length)}else{inputGroup.find('.js-phonemask-result').attr('data-tilda-rule-minlength',1)}}
